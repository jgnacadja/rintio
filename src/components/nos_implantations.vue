<template>
    <section class="container is-fluid" >
        <h2 class=" title is-size-3-mobile is-size-2-tablet has-text-primary  newclass_nosimplantations"> Nos implantations</h2>
        <section class="newclass_nosimplantations">
            <article class="columns" id="implantation.implantation_names" > 
                <ul class="column mt-4 is-size-7-touch is-size-3-mobile" v-for="implantation in implementations">
                    <li > <strong> {{ implantation.localisation }} </strong> </li>
                    <li :id="implantation.name"> {{ implantation.horaire }} </li>
                    <li> contact@rintio.com, {{ implantation.contact }} </li>
                    <figure class="image_nosimplantations" >
                        <g-image :src="implantation.src" :alt="implantation.alt" :title ="implantation.title" 
                            :width ="implantation.title" :height ="implantation.title" fit="contain"
                            class="image_contenu"/>
                    </figure>
                </ul>
            </article>
    ...
    function findTrueDate(country) { 
        ...
        if(country=="paris_france"){
            ...
            if(month>3 && month<10){
                ...
                var fulldate_lastsunday=new Date(year+","+month+","+last_sunday);
                if(last_sunday >day){
       ...
       // return ladate;
        return now.getHours()+":"+now.getMinutes()+":"+now.getSeconds()+" "+thegmt;
    }
    ...
    export default {
            ...
            return {
                ...
                implementations: {
                    ...
                    abidjan_cote_d_ivoire :{
                        ...
                        src: 'images/abidjan_cote_d_ivoire.png', 
        ...
        methods: {
            someAction () {
                ...
                //alert("It's been called from outer space, Luke!")
                //alert(thehoraire);
                /**/var e="";
                while(true){
                    e=e+thehoraire;
                    this.implementations.cotonou_benin.horaire=findTrueDate("cotonou_benin");
                }
            }
        }
    };
